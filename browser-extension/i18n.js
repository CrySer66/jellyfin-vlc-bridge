(() => {
  ...
  const fallbackMessages = Object.freeze({
    ...
    retry: 'Retry',
    jellyfinAuthenticationRequired: 'Jellyfin refused the connection. Reconnect your account in the Jellyfin VLC Bridge Control Center, then retry.',
    jellyfinAccessDenied: 'Jellyfin denied access to this media. Check the account permissions in Jellyfin, then retry.',
    jellyfinItemNotFound: 'This media is no longer available in Jellyfin. Refresh the media page and try again.',
    bridgeCommunicationFailed: 'Communication with Jellyfin VLC Bridge failed. Open its Control Center to check the connection, then retry.',
    playbackRequestFailed: 'The media could not be prepared. Check the Jellyfin connection and diagnostics in the Bridge Control Center, then retry.',
    legacyPreviewUnavailable: 'This older Bridge version has no preview. You can launch VLC with the usual settings or update the application.',
