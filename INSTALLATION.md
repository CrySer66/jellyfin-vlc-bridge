...
[English](INSTALLATION.en.md) · [Retour à l’accueil](README.md)

Suivez les étapes ci-dessous pour installer l’application Windows, ajouter
l’extension Chrome et lancer votre première vidéo. Les options WinGet et
d’installation automatisée sont regroupées à la fin du guide.
...
Pour utiliser un autre serveur, cliquez sur **Changer de serveur Jellyfin** et autorisez le nouveau code Quick Connect. L’ancienne connexion est conservée jusqu’à la réussite de la nouvelle connexion.

## Installer l'extension Chrome

Après une première installation, cliquez sur **Ouvrir Chrome Web Store** dans l’assistant. Sur la fiche de l’extension, cliquez sur **Ajouter à Chrome**, puis confirmez.

Si l’assistant est fermé ou si ce bouton n’est pas affiché après une mise à jour, ouvrez directement la fiche officielle :

[Jellyfin VLC Bridge sur le Chrome Web Store](https://chromewebstore.google.com/detail/hkjbodgdbjhignhlbecchiigcfigpidp)
...
L’installateur ne possède pas encore de signature Windows. Téléchargez-le depuis la page officielle et suivez le [guide de vérification](docs/VERIFY_DOWNLOADS.md) avant de répondre à l’avertissement.
...
Pour désinstaller, utilisez **Paramètres Windows → Applications → Jellyfin VLC Bridge**, comme décrit dans la rubrique [Désinstallation](#désinstallation). WinGet ne remplace ni la gestion de l'extension par Chrome, ni une signature Authenticode : il ne garantit pas la disparition de SmartScreen.
