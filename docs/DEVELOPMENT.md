...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\Build-WindowsRelease.ps1 -Version 1.11.0
```
...
```text
outputs\JellyfinVlcBridge-1.11.0-Setup.exe
outputs\JellyfinVlcBridge-1.11.0-win-x64.zip
```
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\Build-ExtensionPackage.ps1 -Version 1.6.0
```
...
6. créer la Release et joindre ces deux fichiers, avec le tag correspondant, par exemple `v1.11.0`.
